...
const AddCampaign = () => {
    ...
    const handleAddCampaign = e => {
        ...
        const newCampaign = { image, title, type, description, donation, deadline, email, name }
        fetch('https://fund-sphere-server.vercel.app/campaigns', {
            method: "POST",
            headers: {
            ...
            .then(data => {
                console.log(data)
                if (data.insertedId) {
                    Swal.fire("Successfully add campaign!");
                }
                e.target.reset()
            })
    }
    return (
        <div className="max-w-2xl mx-auto p-6 rounded-lg my-20 border border-[#F2B33D] shadow-2xl">
            <Helmet>
                <meta charSet="utf-8" />
                <title>FundSphere - AddCampaign</title>
            </Helmet>
